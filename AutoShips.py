...
class AutoShips:
    ...
    Methods:
        ...
        __get_new_block_for_ship(self, color, direction, orientation, ship_coordinates):
    ...
    def __get_new_block_for_ship(self, color, direction, orientation, ship_coordinates):
        """
        Checks if new individual blocks that are being added to a ship in the previous method
                are within the grid, otherwise changes the direction.
        Args:
            color (int): x or y coordinate to increment/decrement
        ...
        if (color <= 1 - self.offset * (self.orientation - 1) and self.direction == -1) or (
                color >= 10 - self.offset * (self.orientation - 1) and self.direction == 1):
            self.direction *= -1
