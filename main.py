...
class Main:
    def __init__(self):
        ...
        self.play_again_message = "Do you want to play again?"
        self.play_again_button = Button(left_margin + 18 * block_size, "PLAY AGAIN", self.play_again_message)
        self.quit_game_button = Button(0, "QUIT", self.play_again_message, -5)
        ...
        while not self.game_over:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
                    sys.exit()
                elif not self.computer_turn and event.type == pygame.MOUSEBUTTONDOWN and self.quit_game_button.rect.collidepoint(mouse):
                    pygame.quit()
                    sys.exit()
                elif not self.computer_turn and event.type == pygame.MOUSEBUTTONDOWN:
                    x, y = event.pos
                    if 0 < x < 93 and 0 < y < 62:
                        pygame.quit()
                        sys.exit()
